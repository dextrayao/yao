...
describe('evolution', () => {
  it('gates stages on both growth and age', () => {
    const DAY = 24 * 3_600_000;
    expect(stageFor(0, 0)).toBe('egg');
    expect(stageFor(20, 60_000)).toBe('egg'); // growth ok, too young
    expect(stageFor(20, 3 * 3_600_000)).toBe('wisp');
    expect(stageFor(100, 11 * DAY)).toBe('transcendent');
    expect(stageFor(50, 11 * DAY)).toBe('sprite'); // age ok, growth gates
  });
...
describe('render', () => {
  it('produces a self-contained svg string', () => {
    const svg = renderPetSvg(deriveGenome('seed-alpha'), 'spirit');
